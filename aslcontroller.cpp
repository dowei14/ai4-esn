...
//					state=7;
		...
		int stateESN =test->RecurrentNetwork(createInputVectorESN(),createOutputVectorESN(), false);
		if(state!= stateESN) std::cout<<state << " " <<stateESN<<std::endl;
//		state = test->RecurrentNetwork(createInputVectorESN(),createOutputVectorESN(), false);
//		if (runNumber > 1 && state > 0) state = test->RecurrentNetwork(createInputVectorESN(),createOutputVectorESN(), false);
		// execute action
		if (state==0) {
	...
	if (vehicle->getPosition().z<1.0) reset = true;
};
...
// store vectors
std::vector<double> ASLController::createInputVectorESN(){
...
//	for (int i=0;i<12;i++) input.push_back(0.0);
	...
	input.push_back(angleCurrentBox);
	input.push_back(irLeftLong);
	input.push_back(irRightLong);
	input.push_back(irLeftShort);
	input.push_back(irRightShort);
	input.push_back(irFrontLeft);
	input.push_back(irFrontRight);

	if (prevhaveTarget) input.push_back(1.0);
	else input.push_back(0.0);

	return input;
}

std::vector<double> ASLController::createOutputVectorESN(){
